...
UPDATE #DMED SET LG_PODE_ALTERAR = 'N', MOTIVO = 'SOLICITOU RECIBO DE IR' FROM RECIBOIR_CONTRATANTE R WHERE R.CD_CONTRATANTE = #DMED.CD_CONTRATANTE AND R.ANO IN(2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022)
...
-- segunda aba todos os tratamentos E MANUTENCAO --2020 --UMA ABA COM TODOS OS PAGAMENTOS DESSES CPFS DO EXCEL
SELECT Codigo_Contratante,NM_CONTRATANTE,SUM(VALOR_PAGO) VALOR FROM #DMED d
--WHERE DSTPDOCUMENTO = 'MENSALIDADE' --OR LG_PODE_ALTERAR = 'N'
where d.tipo <> 'M'  and d.dstpdocumento in('ORCAMENTO', 'MANUTENCAO')
and exists(select * from ##tt t where cast(d.CGC_CPF as decimal) = cast(t.cpf as decimal)) 
GROUP BY Codigo_Contratante,NM_CONTRATANTE ORDER BY NM_CONTRATANTE


-- terceira  2020 --UMA ABA COM TODOS QUE PEDIRAM INFORME DE RENDIMENTO EM ANOS ANTERIORES
SELECT Codigo_Contratante,NM_CONTRATANTE,SUM(VALOR_PAGO) VALOR FROM #DMED d
--WHERE DSTPDOCUMENTO = 'MENSALIDADE' --OR LG_PODE_ALTERAR = 'N'
where d.tipo <> 'M'  and d.dstpdocumento in('ORCAMENTO', 'MANUTENCAO')
and motivo in('INFORMOU IR NO ORCAMENTO','SOLICITOU RECIBO DE IR')
and not exists(select * from ##tt t where cast(d.CGC_CPF as decimal) = cast(t.cpf as decimal)) 
GROUP BY Codigo_Contratante,NM_CONTRATANTE ORDER BY NM_CONTRATANTE


-- quarta aba  2020 --UMA ABA COM TODOS QUE ENTRARAM NO CONVENIO EM 2019
SELECT Codigo_Contratante,NM_CONTRATANTE,SUM(VALOR_PAGO) VALOR FROM #DMED d
--WHERE DSTPDOCUMENTO = 'MENSALIDADE' --OR LG_PODE_ALTERAR = 'N'
where d.tipo <> 'M'  and d.dstpdocumento in('ORCAMENTO', 'MANUTENCAO')
and motivo in('CONTRATANTE NOVO')
and not exists(select * from ##tt t where cast(d.CGC_CPF as decimal) = cast(t.cpf as decimal)) 
GROUP BY Codigo_Contratante,NM_CONTRATANTE ORDER BY VALOR DESC


-- quinta aba  2020 --UMA ABA COM OS DEMAIS QUE NÃO ENTRARAM NAS TRES ABAS ANTERIORES.
SELECT Codigo_Contratante,NM_CONTRATANTE,SUM(VALOR_PAGO) VALOR FROM #DMED d
--WHERE DSTPDOCUMENTO = 'MENSALIDADE' --OR LG_PODE_ALTERAR = 'N'
where   d.dstpdocumento in('ORCAMENTO', 'MANUTENCAO')
and (motivo not in('CONTRATANTE NOVO','INFORMOU IR NO ORCAMENTO','SOLICITOU RECIBO DE IR')
AND
 not  exists(select * from ##tt t where cast(d.CGC_CPF as decimal) = cast(t.cpf as decimal)) )
GROUP BY Codigo_Contratante,NM_CONTRATANTE ORDER BY VALOR DESC
