...
uses
  ...
  DBCtrls, Db, StdCtrls, Mask, DBTables, Grids, DBGrids, ExtCtrls, Buttons, DBIProcs,
  ADODB, Menus,clipBrd;//, Biblio;
...
type
  TFormTituloReceber = class(TForm)
    ...
    DBCheckBoxDeposito1: TDBCheckBox;
    ...
    CheckBoxDtAtCalcJuros: TCheckBox;
    ADOQuery1: TADOQuery;
    DataSource1: TDataSource;
    QueryTitulonnBoleto: TIntegerField;
    Label31: TLabel;
    Label32: TLabel;
    Label33: TLabel;
    Label34: TLabel;
    EditnnBoleto_original: TEdit;
    EditnnBanco: TEdit;
    EditCpf: TEdit;
    PopupMenu1: TPopupMenu;
    oi1: TMenuItem;
    LocalizarporIDdaRecorrencia1: TMenuItem;
    LocalizarporNmerodoPedidoRecorrencia1: TMenuItem;
    Label35: TLabel;
    ADOQueryRecorrencia: TADOQuery;
    GroupBox1: TGroupBox;
    ADOQueryRecorrenciaId: TAutoIncField;
    ADOQueryRecorrenciaCodigo_Contratante: TIntegerField;
    ADOQueryRecorrenciaOperadora_Recorrencia: TStringField;
    ADOQueryRecorrenciaNome_do_cliente: TStringField;
    ADOQueryRecorrenciaIdentificao_do_cliente: TStringField;
    ADOQueryRecorrenciaEmail_do_cliente: TStringField;
    ADOQueryRecorrenciaTelefone_do_cliente: TStringField;
    ADOQueryRecorrenciaData_do_pedido: TDateTimeField;
    ADOQueryRecorrenciaIdentificador_de_transao_Checkout: TStringField;
    ADOQueryRecorrenciaNumero_do_Pedido_da_Loja: TStringField;
    ADOQueryRecorrenciaId_Recorrencia: TStringField;
    ADOQueryRecorrenciaDia_Recorrencia: TIntegerField;
    ADOQueryRecorrenciaAtivo: TBooleanField;
    ADOQueryRecorrenciaReferente: TStringField;
    ADOQueryRecorrenciaValor: TBCDField;
    ADOQueryRecorrenciaOperadorAlteracao: TStringField;
    ADOQueryRecorrenciaBandeiraCartao: TStringField;
    Label36: TLabel;
    DBEdit1: TDBEdit;
    DataSourceRecorrencia: TDataSource;
    Label37: TLabel;
    DBEdit2: TDBEdit;
    Label38: TLabel;
    DBEdit3: TDBEdit;
    Label39: TLabel;
    DBEdit4: TDBEdit;
    DBNavigatorRecorrencia: TDBNavigator;
    Label40: TLabel;
    DBEdit5: TDBEdit;
    PanelCancel: TPanel;
    EditMotivo: TEdit;
    ...
    procedure CheckBoxDtAtCalcJurosClick(Sender: TObject);
    procedure oi1Click(Sender: TObject);
    procedure LocalizarporIDdaRecorrencia1Click(Sender: TObject);
    procedure LocalizarporNmerodoPedidoRecorrencia1Click(Sender: TObject);
    procedure ADOQueryRecorrenciaAfterOpen(DataSet: TDataSet);
  private
...
uses Senha, Seletor, Contratante, udm, Fundo, PesqTitBaixadoSemRecibo,
  ReimpressaoRecibo , PesquisaContratante, SimulacaoPgtit, PesqLogContrat,
  fmLogReceber;
...
//Antiga
     ...
     CheckBoxDtAtCalcJuros.Checked := true;
...
begin
     ...
     try
      ...
      QueryTitulo.Post
       ...
       begin
         ShowMessage('Erro Gravação da Tabela de Títulos Abertos . '  + e.message);
         QueryTitulo.Cancel;
         Exit;
       end;
     end;
...
begin
     if not QueryTituloNumero_Titulo.IsNull then
      QueryTitulonnBoleto.AsString := dm.execmd('select nnboleto from faturamento where desdobramento = ''' + QueryTituloDesdobramento.AsString + ''' and numero_titulo = ' + QueryTituloNumero_Titulo.asstring,'nnboleto');
...
begin
   ...
   if CheckBoxDtAtCalcJuros.Checked then
      DataBaixa := dm.Date
   else
      DataBaixa := StrToDate(MaskEditDataBaixa.Text);
   ...
   begin
      sp_calcJuros.Close;
      sp_calcJuros.Parameters[2].Value  := QueryTitulo.FieldByName('desdobramento').AsString;
      sp_calcJuros.Parameters[1].Value  := QueryTitulo.FieldByName('numero_titulo').AsInteger;

   if CheckBoxDtAtCalcJuros.Checked then
      sp_calcJuros.Parameters[3].Value  := dm.GetDataSrv()
   else
      sp_calcJuros.Parameters[3].Value  := MaskEditDataBaixa.Text;
...
begin
     ...
     try
     if DBGridContratante.CanFocus then
...
// Verifica se o título já foi Cancelado ou não
     ...
     try
       ...
       begin
         ShowMessage('Erro Gravação da Tabela de Títulos Cancelados. ' + e.message);
     ...
     try
       dm.execmd('delete receber_formaLiqCNAB where numero_titulo = ' + QueryTituloNumero_Titulo.AsString + ' and desdobramento = ''' + QueryTituloDesdobramento.AsString + '''');
       QueryTitulo.Delete;
     except on E: Exception do
...
begin
   ...
   if QueryTituloData_Vencimento.IsNull then
   begin
      ShowMessage('Data do vencimento deve ser obrigatória !');
      DBEditVencimento.SetFocus;
      SysUtils.abort;
   end;
...
begin
  ...
  begin
     ...
     dm.execmd('select nnBoleto,nnBoleto_original, CGC_CPF from faturamento f, contratante c where f.numero_titulo = ' + QueryTituloNumero_Titulo.AsString + ' and f.desdobramento = ''' + QueryTituloDesdobramento.AsString  + ''' and f.Codigo_Contratante = c.codigo','nnBoleto');
     EditnnBanco.Text := dm.QueryGlobal.FieldByName('nnBoleto').AsString;
     EditnnBoleto_original.Text := dm.QueryGlobal.FieldByName('nnBoleto_original').AsString;
     EditCpf.Text := dm.QueryGlobal.FieldByName('CGC_CPF').AsString;
...
begin
    ...
    if not  QueryContratanteCodigo.IsNull then
    begin
       // a rotina verificaMsg espera um codigo de usuario, entao eu pego um aqui nesse selecte..
       cdUsuAux := dm.execmd('select top 1 codigo_completo c from usuario where codigo = ' +  QueryContratanteCodigo.AsString +
                             ' union select top 1 codigo_completo c from usuario where contratante_titular = ' + QueryContratanteCodigo.AsString,'c');
       Dm.VerificaMsg(cdUsuAux,'T');

     ADOQueryRecorrencia.Close;
     ADOQueryRecorrencia.Parameters.ParamByName('codigo').Value:=QueryContratanteCodigo.AsString;
     ADOQueryRecorrencia.Open;
    end;
end;
...
procedure TFormTituloReceber.QueryTituloAfterOpen(DataSet: TDataSet);
begin
   LabelOpUltAlt.Caption := '';
end;

procedure TFormTituloReceber.BitBtnLogAltClick(Sender: TObject);
begin
...
procedure TFormTituloReceber.CheckBoxDtAtCalcJurosClick(Sender: TObject);
begin
  if MaskEditDataBaixa.Text <> '  /  /    ' then
  begin
    MaskEditDataBaixaExit(sender);
    DBEditValorPagamentoEnter(sender);
  end;
end;

procedure TFormTituloReceber.oi1Click(Sender: TObject);
var
  Cod,TextoPesquisado:string;
begin
  TextoPesquisado:=InputBox('CPF / CNPJ','Digite o CPF/CNPJ para localizar o contratante','');
  Cod:=DM.ExecutaComando('select Codigo  a from contratante where CGC_CPF  = '''+TextoPesquisado+'''','a');
  if Cod<>'' then
  begin
    with QueryContratante do
       begin
         Close;
         SQL[1] := 'where CGC_CPF  = '''+TextoPesquisado+'''';
         SQL[2] := 'Order by Codigo';
         Open;
       end;
  end
  else
    ShowMessage('Não foi localizado nenhum contratante com esse CPF/CNPJ');
end;

procedure TFormTituloReceber.LocalizarporIDdaRecorrencia1Click(
  Sender: TObject);
var
  Cod,TextoPesquisado:string;
begin
  TextoPesquisado:=InputBox('ID Recorrencia','Digite o Número do ID da Recorrencia para localizar o contratante','');
...
procedure TFormTituloReceber.LocalizarporNmerodoPedidoRecorrencia1Click(
  Sender: TObject);
var
  Cod,TextoPesquisado:string;
begin
...
procedure TFormTituloReceber.ADOQueryRecorrenciaAfterOpen(
  DataSet: TDataSet);
begin
    if ADOQueryRecorrencia.RecordCount>1 then
      DBNavigatorRecorrencia.Visible:=true
    else
      DBNavigatorRecorrencia.Visible:=false;
end;
