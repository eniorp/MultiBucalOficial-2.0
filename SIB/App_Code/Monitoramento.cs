...
public class Monitoramento
{
    ...
    public string geraMonitoramento(string periodo, string cdANS, string operador, string nrLote, string tpGeracao, string vlfs)
    {
        ...
        //schemaLocation.Value = "http://www.ans.gov.br/padroes/tiss/schemas/tissMonitoramentoV3_03_03.xsd";
        schemaLocation.Value = "http://www.ans.gov.br/padroes/tiss/schemas/tissMonitoramentoV1_00_00.xsd";
        ...
        //geraCabecalhoMonit(mensagemEnvioANS, DateTime.Today.ToString("yyyy-MM-dd"), DateTime.Now.ToString("hh:mm:ss"), cdANS, "3.03.03", nrLote, periodo);
        geraCabecalhoMonit(mensagemEnvioANS, DateTime.Today.ToString("yyyy-MM-dd"), DateTime.Now.ToString("hh:mm:ss"), cdANS, "1.00.00", nrLote, periodo);
        ...
        SqlParameter vlFs;
        vlFs = cmd.Parameters.Add(new SqlParameter("@vlFs", SqlDbType.Money));
        vlFs.Direction = ParameterDirection.Input;
        vlFs.Value = vlfs;
    ...
    private void geraGuiasMonitoramento(XmlElement operadoraParaANS, SqlDataReader dr)
    {
        ...
        while (dr.Read())
        {
            ...
            if (dr["tipoPessoa"].ToString() == "PF")
                cpfCnpjBase = dr["cpf"].ToString();
            else
                cpfCnpjBase = dr["cnpj"].ToString();


            if (nrGuiaOperadoraAux != dr["nrGuiaOperadora"].ToString() || cpfCnpjAux != cpfCnpjBase)
            {
    ...
    private XmlElement geraDetGuiaMonit(XmlElement operadoraParaANS, XmlElement guiaMonitoramento, SqlDataReader dr)
    {
        ...
        versaoTISSPrestador.InnerText = "019";
        guiaMonitoramento.AppendChild(versaoTISSPrestador);
        XmlElement formaEnvio = docXml.CreateElement("ans", "formaEnvio", xsd);
        formaEnvio.InnerText = "4"; //
        guiaMonitoramento.AppendChild(formaEnvio);
        ...
        else
        {
            ...
            XmlElement sexo = docXml.CreateElement("ans", "sexo", xsd);
            sexo.InnerText = dr["sexo"].ToString();
            identBeneficiario.AppendChild(sexo);
            XmlElement dataNascimento = docXml.CreateElement("ans", "dataNascimento", xsd);
            dataNascimento.InnerText = dr["nasc"].ToString();
            identBeneficiario.AppendChild(dataNascimento);
            XmlElement municipioResidencia = docXml.CreateElement("ans", "municipioResidencia", xsd);
            municipioResidencia.InnerText = dr["cidade_residencial"].ToString();
            identBeneficiario.AppendChild(municipioResidencia);
        ...
        {
            valorTotalInformado.InnerText = "0";
            valorProcessado.InnerText = "0";
            valorTotalPagoProcedimentos.InnerText = "0";
            valorPagoGuia.InnerText = "0";

            System.IO.StreamWriter arq = new System.IO.StreamWriter(@"c:\temp\monit.txt", true);
            arq.WriteLine("erro : " + ex.Message + " guia prest " + numeroGuia_prestador + " guia " + dr["nrGuiaOperadora"].ToString() + " valor : " + dr["valorTot"].ToString());
            arq.Close();
        ...
        XmlElement valorTotalCoParticipacao = docXml.CreateElement("ans", "valorTotalCoParticipacao", xsd);

        valorTotalCoParticipacao.InnerText = String.Format("{0:0.00}", decimal.Parse(dr["valorTotalCoParticipacao"].ToString())).Replace(",", ".");
        ...
        valoresGuia.AppendChild(valorTotalCoParticipacao);
    ...
    private XmlElement geraProcedimento(XmlElement guiaMonitoramento, SqlDataReader dr)
    {
        ...
        XmlElement valorCoParticipacao = docXml.CreateElement("ans", "valorCoParticipacao", xsd);
        valorCoParticipacao.InnerText = String.Format("{0:0.00}", decimal.Parse(dr["vrCooparticipacao"].ToString())).Replace(",", ".");
        procedimentos.AppendChild(valorPagoFornecedor);
        procedimentos.AppendChild(valorCoParticipacao);
